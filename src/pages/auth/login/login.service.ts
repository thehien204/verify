...
import {
  ConnectTokenIdentityResultDto,
  ExternalAuthenticateModel,
  ExternalLoginProviderInfoModel,
  TokenAuthServiceProxy
} from "@service-proxies/verify-service-proxies";
import {ScriptLoaderService} from "../../../shared/utils/ScriptLoaderService";
import {UrlHelper} from "../../../shared/helpers/UrlHelper";
...
export class LoginService {
  ...
  // authenticate(finallyCallback?: (status?:boolean) => void, redirectUrl?: string, captchaResponse?: string): void {
  ...
  private facebookLoginStatusChangeCallback(resp) {
    if (resp.status === 'connected') {
      ...
      this._tokenAuthService.externalAuthenticate(model)
        .subscribe((result: ConnectTokenIdentityResultDto) => {
          // if (result.waitingForActivation) {
          //   // console.warn('You have successfully registered. Waiting for activation!');
          //   return;
          // }
