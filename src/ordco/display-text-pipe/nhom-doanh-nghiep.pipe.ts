...
@Pipe({
  name: 'nhomDoanhNghiepDisplayText'
})
export class NhomDoanhNghiepPipe implements PipeTransform {
  ...
  transform(value: string | number): Observable<string> {
    const namePipe = 'nhomDoanhNghiep';
    this.dataService.getDataOptions(this.proxy.nhomDoanhNghiep(), namePipe);
