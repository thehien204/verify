...
@Pipe({
  name: 'tenThongBaoDisplayText'
})
export class TenThongBaoPipe implements PipeTransform {
  ...
  transform(value: string | number): Observable<string> {
    const namePipe = 'tenThongBao';
    this.dataService.getDataOptions(this.proxy.listNotificationName(), namePipe);
