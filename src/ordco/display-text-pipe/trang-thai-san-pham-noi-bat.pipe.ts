...
@Pipe({
  name: 'trangThaiSanPhamNoiBatDisplayText'
})
export class TrangThaiSanPhamNoiBatPipe implements PipeTransform {
  ...
  transform(value: string | number): Observable<string> {
    const namePipe = 'trangThaiSanPhamNoiBat';
    this.dataService.getDataOptions(this.proxy.trangThaiSanPhamNoiBat(), namePipe);
